...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.geometry("360x270")  # Dimensions of the form
    ...
    def show_guest_form(self):
        ...
        self.lbl_event.grid(row=1, column=0, pady=(10, 0), padx=(0, 10))
    ...
    def handle_guest_submission(self):
        ...
        if name and email and host_id:
            self.add_guest_to_event(name, email, host_id, guest_event)
        else:
            messagebox.showwarning("Input Error", "Please fill in all fields.")

    # Function to add guest to the event
    def add_guest_to_event(self, name, email, client_id, guest_event):
        conn = sqlite3.connect("events.db")
        c = conn.cursor()
        c.execute("INSERT INTO guests (event_title, name, email, client_id ) VALUES (?, ?, ?, ?)", (guest_event, name, email,client_id ))
        conn.commit()
        conn.close()
