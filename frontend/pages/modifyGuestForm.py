...
class App(customtkinter.CTk):
    ...
    def update_event(self):
        updated_client_id = self.entry_host_id.get()
        updated_category = self.entry_event_type.get()
        updated_guest_name = self.guest_name_entry.get()
        updated_guest_email = self.guest_email_entry.get()

        if not all([updated_category, updated_guest_name, updated_guest_email, updated_client_id]):
            messagebox.showerror("Input Error", "Please fill all fields.")
            return

        conn = sqlite3.connect("events.db")
        c = conn.cursor()
        c.execute("UPDATE guests SET event_title=?,  name=?,  email=? WHERE client_id=?", 
                  (updated_category, updated_guest_name, updated_guest_email, updated_client_id))
        conn.commit()
        conn.close()
